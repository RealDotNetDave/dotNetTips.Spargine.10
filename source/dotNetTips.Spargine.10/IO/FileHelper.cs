...
public static class FileHelper
{
	...
	public static long CalculateTotalFileSize(ReadOnlySpan<FileInfo> files)
	{
		...
		{
			totalFileLength += ComputeFileSizeContribution(file);
		}
	...
	/// </code>
	/// </example>
	[Information(nameof(DownloadFileFromWebAsync), UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Completed, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
	...
	private static bool EvaluateAccessRules(AuthorizationRuleCollection rules, FileSystemRights permission)
	{
		...
		{
			if (RuleMatchesPermissionAndAllows(rule, permission))
			{
				allow = true;
			}
			else if (RuleMatchesPermissionAndDenies(rule, permission))
			{
				deny = true;
			}
		}
	...
	private static async Task UnWinZipAsync([NotNull] string zipPath, [NotNull] string expandedDirectoryPath, CancellationToken cancellationToken = default)
	{
		...
		{
			await ExtractZipEntryAsync(zipArchiveStream.Entries[zipArchiveCount], expandedDirectoryPath, fullExpandedPath, cancellationToken).ConfigureAwait(false);
		}
	}
