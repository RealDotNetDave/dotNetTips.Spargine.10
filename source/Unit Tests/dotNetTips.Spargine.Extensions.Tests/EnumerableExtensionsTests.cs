...
public class EnumerableExtensionsTests
{
	...
	[TestMethod]
	public void FastShuffleAsSpan_CanAccessElementsByIndex()
	{
		var numbers = Enumerable.Range(1, 10);

		var shuffledSpan = numbers.FastShuffleAsSpan();

		// Verify we can access all elements by index
		for (var i = 0; i < shuffledSpan.Length; i++)
		{
			var element = shuffledSpan[i];
			Assert.IsTrue(element >= 1 && element <= 10);
		}
	}

	[TestMethod]
	public void FastShuffleAsSpan_EmptyCollection_ReturnsEmptySpan()
	{
		var emptyList = new List<int>();

		var shuffledSpan = emptyList.FastShuffleAsSpan();

		Assert.AreEqual(0, shuffledSpan.Length);
	}

	[TestMethod]
	public void FastShuffleAsSpan_MultipleInvocations_ProducesDifferentOrders()
	{
		var numbers = Enumerable.Range(1, 100);

		var firstShuffle = numbers.FastShuffleAsSpan().ToArray();
		var secondShuffle = numbers.FastShuffleAsSpan().ToArray();

		// Statistically, two shuffles should produce different orders
		var identical = firstShuffle.SequenceEqual(secondShuffle);
		Assert.IsFalse(identical);
	}

	[TestMethod]
	public void FastShuffleAsSpan_NullCollection_ThrowsArgumentNullException()
	{
		IEnumerable<int> nullCollection = null;

		_ = Assert.ThrowsExactly<ArgumentNullException>(() => nullCollection.FastShuffleAsSpan());
	}

	[TestMethod]
	public void FastShuffleAsSpan_SingleElement_ReturnsSingleElementSpan()
	{
		var singleItem = new List<int> { 42 };

		var shuffledSpan = singleItem.FastShuffleAsSpan();

		Assert.AreEqual(1, shuffledSpan.Length);
		Assert.AreEqual(42, shuffledSpan[0]);
	}

	[TestMethod]
	public void FastShuffleAsSpan_SmallCollection_ShufflesCorrectly()
	{
		...
		Assert.AreEqual(5, shuffledSpan.Length);

		// Verify all elements are present
		foreach (var num in originalNumbers)
		{
			Assert.IsTrue(shuffledSpan.ToArray().Contains(num));
		}
	}

	[TestMethod]
	public void FastShuffleAsSpan_WithComplexTypes_PreservesAllElements()
	{
		var people = RandomData.GeneratePersonRefCollection(Count);
		var originalPeople = people.ToHashSet();

		var shuffledSpan = people.FastShuffleAsSpan();

		Assert.AreEqual(Count, shuffledSpan.Length);

		// Verify all elements are present
		foreach (var person in originalPeople)
		{
			Assert.IsTrue(shuffledSpan.ToArray().Contains(person));
		}
	}

	[TestMethod]
	public void FastShuffleAsSpan_WithDuplicates_PreservesAllElements()
	{
		var numbers = new List<int> { 1, 2, 2, 3, 3, 3, 4, 4, 4, 4 };

		var shuffledSpan = numbers.FastShuffleAsSpan();

		Assert.AreEqual(10, shuffledSpan.Length);

		// Count occurrences
		var shuffledArray = shuffledSpan.ToArray();
		Assert.AreEqual(1, shuffledArray.Count(n => n == 1));
		Assert.AreEqual(2, shuffledArray.Count(n => n == 2));
		Assert.AreEqual(3, shuffledArray.Count(n => n == 3));
		Assert.AreEqual(4, shuffledArray.Count(n => n == 4));
	}

	[TestMethod]
	public void FastShuffleAsSpan_WithStrings_ShufflesCorrectly()
	{
		var words = RandomData.GenerateWords(Count, 5, 10);
		var originalWords = words.ToHashSet();

		var shuffledSpan = words.FastShuffleAsSpan();
		...
		foreach (var word in originalWords)
		{
			Assert.IsTrue(shuffledSpan.ToArray().Contains(word));
		}
	}

	[TestMethod]
	public void FastShuffleAsSpan_WithValueTypes_ShufflesCorrectly()
	{
		var numbers = Enumerable.Range(1, 100);
		var originalNumbers = numbers.ToHashSet();
		...
		Assert.AreEqual(100, shuffledSpan.Length);
