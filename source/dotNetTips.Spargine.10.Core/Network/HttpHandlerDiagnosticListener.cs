...
public sealed class HttpHandlerDiagnosticListener(ILogger logger) : IObserver<KeyValuePair<string, object>>
{
	...
	{
		var requestDataType = Type.GetType("System.Net.Http.DiagnosticsHandler+ActivityStartData, System.Net.Http", throwOnError: true) ?? throw new InvalidOperationException(Resources.TheTypeSystemNetHttpDiagnosticsHandlerActi1);

		var requestProperty = requestDataType.GetProperty("Request");
		return requestProperty == null
			? throw new InvalidOperationException(Resources.ThePropertyRequestCouldNotBeFoundOnTheType)
			: (o =>
		{
			var value = requestProperty.GetValue(o) ?? throw new InvalidOperationException("The 'Request' property returned a null value.");
	...
	{
		var requestDataType = Type.GetType("System.Net.Http.DiagnosticsHandler+ActivityStopData, System.Net.Http", throwOnError: true) ?? throw new InvalidOperationException(Resources.TheTypeSystemNetHttpDiagnosticsHandlerActi);

		var requestProperty = requestDataType.GetProperty("Response");

		return requestProperty == null
			? throw new InvalidOperationException(Resources.ThePropertyResponseCouldNotBeFoundOnTheTyp)
			: (o =>
		{
			var value = requestProperty.GetValue(o);
			return value is null
				? throw new InvalidOperationException("The 'Response' property returned a null value.")
	...
	public void OnNext([DisallowNull] KeyValuePair<string, object> value)
	{
