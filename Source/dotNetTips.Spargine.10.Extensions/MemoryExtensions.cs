...
// </copyright>
// <summary>
// Extension methods for <c>Memory{T}</c> providing presence checks (<c>IsNotEmpty</c>),
...
/// <summary>
/// Provides extension methods for <see cref="Memory{T}"/> to enhance its functionality.
/// </summary>
/// <remarks>
...
/// low-allocation equality comparison (<c>IsEqualTo</c>) performed over the underlying <see cref="Span{T}"/>.
/// </remarks>
...
public static class MemoryExtensions
{
	...
	/// <param name="memory">The <see cref="Memory{T}"/> instance to extend.</param>
	extension<T>([DisallowNull] Memory<T> memory)
	{
		/// <summary>
		/// Determines whether the <see cref="Memory{T}"/> contains any elements.
		/// </summary>
		...
		[Information(nameof(IsNotEmpty), "Copilot Agent", "07-08-2026", UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Optimize, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
		...
		/// <summary>
		/// Attempts to retrieve the underlying <see cref="ArraySegment{T}"/> that backs this <see cref="Memory{T}"/> without copying.
		/// </summary>
		/// <param name="segment">
		...
		/// </example>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(TryGetArraySegment), "Copilot Agent", "07-08-2026", UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Optimize, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
		...
		/// <summary>
		/// Returns the elements of this <see cref="Memory{T}"/> as an array, avoiding a copy when possible.
		/// </summary>
		/// <returns>
		...
		/// this falls back to <see cref="Memory{T}.ToArray"/>, which always allocates a new array.
		/// </remarks>
		/// <example>
		/// <code>
		/// int[] source = { 1, 2, 3 };
		/// Memory&lt;int&gt; full = source;
		/// int[] result = full.ToArrayIfNeeded();
		/// Console.WriteLine(ReferenceEquals(source, result)); // Output: True
		///
		/// Memory&lt;int&gt; slice = source.AsMemory(1, 2);
		...
		[return: NotNull]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(ToArrayIfNeeded), "Copilot Agent", "07-08-2026", UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Optimize, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
		...
		/// <summary>
		/// Determines whether the current <see cref="Memory{T}"/> is equal to <paramref name="other"/> by comparing elements.
		/// </summary>
		/// <param name="other">The <see cref="Memory{T}"/> instance to compare with the current memory.</param>
		/// <param name="comparer">
		...
		/// </returns>
		/// <remarks>
		/// For performance, this method compares the underlying <see cref="Span{T}"/> views via
		/// <see cref="System.MemoryExtensions.SequenceEqual{T}(ReadOnlySpan{T}, ReadOnlySpan{T}, IEqualityComparer{T}?)"/>
		/// without additional allocations.
		/// </remarks>
		[Pure]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(IsEqualTo), "Copilot Agent", "07-08-2026", UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Optimize, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
