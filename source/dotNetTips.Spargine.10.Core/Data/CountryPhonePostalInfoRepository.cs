...
public static class CountryPhonePostalInfoRepository
{
	...
	public static CountryPhonePostalInfo? GetCountryPhonePostalInfo([NotNull] string countryNameOrIso)
	{
		...
		return countryNameOrIso.Length switch
		{
			2 => countryData.FirstOrDefault(p => string.Equals(p.Iso2, countryNameOrIso, StringComparison.OrdinalIgnoreCase)),
			3 => countryData.FirstOrDefault(p => string.Equals(p.Iso3, countryNameOrIso, StringComparison.OrdinalIgnoreCase)),
