...
public static class BenchmarkHelper
{
	...
	/// When set to <c>true</c>, the aggregated timing summary generated by <see cref="PerformanceStopwatch"/>
	/// is persisted to a text file in the BenchmarkDotNet artifacts directory (as defined by
	/// <see cref="IConfig.ArtifactsPath"/>). When <c>false</c>, the summary is only written to the console.
	...
	/// </para>
	/// <para>
	/// For each benchmark type, the method:
	/// </para>
	/// <list type="number">
	/// <item><description>Logs the benchmark type name and index to the console.</description></item>
	/// <item><description>Records a diagnostic entry indicating the benchmark start time.</description></item>
	/// <item><description>Executes <see cref="BenchmarkRunner.Run(Type, IConfig)"/> for the type.</description></item>
	/// <item><description>Calculates and records the elapsed time for the benchmark into the <see cref="PerformanceStopwatch"/> laps.</description></item>
	/// <item><description>Adds a diagnostic entry with the formatted runtime using <c>MillisecondsToString()</c>.</description></item>
	/// </list>
	/// <para>
	/// After all benchmarks complete, a detailed summary report is generated via <see cref="PerformanceStopwatch.GetSummaryReport()"/>
	/// and written to the console. If <paramref name="saveResults"/> is <c>true</c>, the summary is also saved to a file
	/// in the BenchmarkDotNet artifacts directory using <see cref="SaveReportToFile(IConfig, string)"/>.
	/// </para>
	/// </remarks>
	/// <example>
	/// <code>
	...
	/// var config = ManualConfig.ToUniqueCollection(DefaultConfig.Instance);
	/// 
	/// // Run specific benchmarks and save timing summary to BenchmarkDotNet artifacts
	/// BenchmarkHelper.RunBenchmarks(
	///     config,
	///     saveResults: true,
	///     typeof(StringExtensionsBenchmark),
	///     typeof(CollectionBenchmark));
	/// 
	/// // Run benchmarks without saving the external timing summary file
	/// BenchmarkHelper.RunBenchmarks(
	///     config,
	///     saveResults: false,
	///     typeof(StringExtensionsBenchmark));
	...
	public static void RunBenchmarks([DisallowNull] IConfig config, bool saveResults = true, [DisallowNull] params Type[] benchmarks)
	{
		config = config.ArgumentNotNull();
	...
	/// <c>SpargineTimingSummary_{timestamp}.txt</c>, where <c>timestamp</c> is produced by
	/// <see cref="DotNetTips.Spargine.Extensions.DateTimeExtensions.ToFriendlyDateString(DateTime)"/>.
	/// </para>
	/// <para>
