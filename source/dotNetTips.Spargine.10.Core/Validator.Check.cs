...
public static partial class Validator
{
	...
	public static bool CheckExists([DisallowNull] this FileInfo input, in bool throwException = false, string errorMessage = ControlChars.EmptyString)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Information(nameof(CheckExists), "David McCarter", "1/31/2022", UnitTestStatus = UnitTestStatus.Completed, BenchmarkStatus = BenchmarkStatus.NotRequired, Status = Status.Available)]
	public static bool CheckExists([DisallowNull] this DirectoryInfo input, in bool createDirectory = false, in bool throwException = false, string errorMessage = ControlChars.EmptyString)
	{
		input = input.ArgumentNotNull();
	...
	public static bool CheckIsCondition<T>([DisallowNull] this T input, in bool condition, in bool throwException = false, string errorMessage = ControlChars.EmptyString)
	{
		input = input.ArgumentNotNull();
...
#pragma warning restore CS8604 // Possible null reference argument.
	...
	/// <summary>
	/// Checks whether the specified <see cref="string"/> is not <c>null</c> or empty.
	/// </summary>
	/// <param name="input">The string to validate.</param>
	/// <param name="throwException">
	/// If set to <c>true</c>, throws an <see cref="ArgumentNullException"/> when <paramref name="input"/> is <c>null</c> or empty.
	/// </param>
	/// <param name="errorMessage">
	/// The error message to use if an exception is thrown. If not provided, a default message is used.
	/// </param>
	/// <returns>
	/// <c>true</c> if <paramref name="input"/> is not <c>null</c> or empty; otherwise, <c>false</c>.
	/// </returns>
	...
	public static bool CheckIsNotNullOrEmpty(this string input, in bool throwException = false, string errorMessage = ControlChars.EmptyString)
	{
		...
		if (isValid is false && throwException)
		{
			ExceptionThrower.ThrowArgumentNullException(CreateExceptionMessage(errorMessage, Resources.StringIsNullOrEmpty));
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Information(nameof(CheckItemsExists), "David McCarter", "4/14/2022", UnitTestStatus = UnitTestStatus.Completed, BenchmarkStatus = BenchmarkStatus.Completed, Status = Status.Available)]
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Information(nameof(CheckTypeEquals), "David McCarter", "1/31/2022", UnitTestStatus = UnitTestStatus.Completed, BenchmarkStatus = BenchmarkStatus.NotRequired, Status = Status.Available)]
	public static bool CheckTypeEquals(this Type input, Type expectedType, in bool throwException = false, string errorMessage = ControlChars.EmptyString)
	{
