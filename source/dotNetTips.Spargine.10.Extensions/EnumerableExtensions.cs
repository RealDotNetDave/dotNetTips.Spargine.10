...
public static class EnumerableExtensions
{
	...
	/// <param name="collection">The enumerable collection to extend with additional functionality.</param>
	/// <remarks>
	/// <para>
	/// This extension block provides a comprehensive set of methods for working with <see cref="IEnumerable{T}"/> collections,
	/// including filtering, transformation, aggregation, ordering, conversion, and validation operations.
	/// </para>
	/// <para>
	/// <b>Performance Optimizations (.NET 10):</b>
	/// </para>
	/// <list type="bullet">
	/// <item><description><b>Type-specific optimizations:</b> Many methods provide specialized implementations for <see cref="List{T}"/>, arrays, and <see cref="ICollection{T}"/> to maximize performance.</description></item>
	/// <item><description><b>Span and memory access:</b> Methods leverage <see cref="CollectionsMarshal.AsSpan{T}(List{T})"/> and <see cref="MemoryMarshal.GetArrayDataReference{T}(T[])"/> where applicable for direct memory access.</description></item>
	/// <item><description><b>Deferred execution:</b> LINQ-style methods use yield return for lazy evaluation to minimize memory allocations.</description></item>
	/// <item><description><b>Expression caching:</b> Methods like <see cref="OrderBy{T}(IEnumerable{T}, string)"/> cache compiled expressions to avoid reflection overhead on subsequent calls.</description></item>
	/// </list>
	/// <para>
	/// <b>Key Method Categories:</b>
	/// </para>
	/// <list type="bullet">
	/// <item><description><b>Search and Index:</b> <see cref="IndexOf{T}(IEnumerable{T}, T)"/>, <see cref="FirstOrNull{T}(IEnumerable{T}, Func{T, bool})"/></description></item>
	/// <item><description><b>Ordering:</b> <see cref="OrderBy{T}(IEnumerable{T}, string)"/>, <see cref="OrderByOrdinal{T}(IEnumerable{T}, Func{T, string})"/></description></item>
	/// <item><description><b>Transformation:</b> <see cref="ReplaceIf{T}(IEnumerable{T}, Func{T, int, bool}, T)"/>, <see cref="FastProcessor{T}(IEnumerable{T}, Action{T})"/></description></item>
	/// <item><description><b>Aggregation:</b> <see cref="FastLongCount{T}(IEnumerable{T})"/>, <see cref="FastAny{T}(IEnumerable{T}, Func{T, bool})"/></description></item>
	/// <item><description><b>Partitioning:</b> <see cref="Partition{T}(IEnumerable{T}, int)"/>, <see cref="FastShuffle{T}(IEnumerable{T})"/></description></item>
	/// <item><description><b>Modification:</b> <see cref="AddFirst{T}(IEnumerable{T}, T)"/>, <see cref="AddLast{T}(IEnumerable{T}, T)"/>, <see cref="Upsert{T}(IEnumerable{T}, T)"/></description></item>
	/// <item><description><b>Validation:</b> <see cref="IsEmpty{T}(IEnumerable{T})"/>, <see cref="IsNotEmpty{T}(IEnumerable{T})"/>, <see cref="HasDuplicates{T}(IEnumerable{T})"/></description></item>
	/// <item><description><b>Conversion:</b> <see cref="ToFrozenSet{T}(IEnumerable{T})"/>, <see cref="ToImmutableList{T}(IEnumerable{T})"/>, <see cref="ToReadOnlyCollection{T}(IEnumerable{T})"/></description></item>
	/// <item><description><b>Comparison:</b> <see cref="StartsWith{T}(IEnumerable{T}, IEnumerable{T})"/>, <see cref="StructuralSequenceEqual{T}(IEnumerable{T}, IEnumerable{T})"/></description></item>
	/// </list>
	/// <para>
	/// <b>Thread Safety:</b> Unless explicitly documented otherwise, these extension methods do not provide thread safety.
	/// If you need to process collections concurrently, consider using thread-safe collection types or appropriate synchronization mechanisms.
	/// </para>
	/// <para>
	/// <b>Null Handling:</b> Most methods validate input parameters using <c>ArgumentNotNull()</c> and throw <see cref="ArgumentNullException"/>
	/// for null inputs, following defensive programming practices.
	/// </para>
	/// </remarks>
	/// <example>
	/// This example demonstrates common usage patterns for IEnumerable extension methods:
	/// <code>
	/// // Searching and filtering
	/// var numbers = new List&lt;int&gt; { 1, 2, 3, 4, 5 };
	/// int index = numbers.IndexOf(3);  // Returns 2
	/// int? firstEven = numbers.FirstOrNull(n =&gt; n % 2 == 0);  // Returns 2
	/// 
	/// // Ordering
	/// var people = new List&lt;Person&gt; { /* ... */ };
	/// var sorted = people.OrderBy("Age desc");
	/// 
	...
	/// ImmutableList&lt;int&gt; immutable = numbers.ToImmutableList();
	/// 
	/// // Validation
	/// bool hasItems = numbers.IsNotEmpty();
	/// bool hasDupes = numbers.HasDuplicates();
	/// 
	/// // Modification (returns new collection)
	/// var withNew = numbers.AddLast(6);
	/// var withFirst = numbers.AddFirst(0);
	/// </code>
	/// </example>
	extension<T>([DisallowNull] IEnumerable<T> collection)
	{
		...
		[return: NotNull]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(ToCollection), "David McCarter", "4/13/2021", OptimizationStatus = OptimizationStatus.Completed, BenchmarkStatus = BenchmarkStatus.Completed, UnitTestStatus = UnitTestStatus.Update, Status = Status.Available)]
		...
		[Pure]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(FastLongCount), "David McCarter", "5/21/2022", OptimizationStatus = OptimizationStatus.Completed, BenchmarkStatus = BenchmarkStatus.Completed, UnitTestStatus = UnitTestStatus.Update, Status = Status.Available)]
		...
		[Pure]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		[Information(nameof(FastLongCount), "David McCarter", "11/21/2020", OptimizationStatus = OptimizationStatus.Completed, BenchmarkStatus = BenchmarkStatus.Completed, UnitTestStatus = UnitTestStatus.Update, Status = Status.Available)]
